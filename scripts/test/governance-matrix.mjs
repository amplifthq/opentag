...
function vitestCommand(...testFiles) {
  return {
    executable: "corepack",
    args: ["pnpm", "vitest", "run", ...testFiles]
  };
}
...
const cases = [
  ...
  },
  {
    ...
    command: vitestCommand("packages/dispatcher/test/replay-harness.test.ts"),
    covers: [
  ...
  },
  {
    ...
    command: {
      executable: "node",
      args: [
        "scripts/test/privacy-redaction-scan.mjs",
        "--allow-missing",
...
function commandExists(command) {
  ...
  if (isAbsolute(command)) {
    ...
    candidates = [resolve(rootDir, command)];
  } else {
    candidates = (process.env.PATH ?? "")
      .split(delimiter)
      .filter(Boolean)
      .flatMap((directory) => {
        const base = resolve(rootDir, directory, command);
...
export function formatCommand(command) {
  return [command.executable, ...command.args]
    .map((argument) => /^[a-z0-9_./:@=+-]+$/iu.test(argument) ? argument : JSON.stringify(argument))
    .join(" ");
}

export function preflight(testCase) {
  const missing = [];
  if (!commandExists(testCase.command.executable)) {
    missing.push(`command:${testCase.command.executable}`);
  }
