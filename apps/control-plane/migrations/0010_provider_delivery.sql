CREATE TABLE cp_provider_delivery_intent (
  ...
  CONSTRAINT cp_provider_delivery_intent_shape_check CHECK(
    (state='pending' AND lease_owner IS NULL AND lease_expires_at IS NULL AND lease_fence IS NULL AND lease_fence_digest IS NULL AND installation_begin_marker_id IS NULL AND installation_begin_marker_digest IS NULL AND scope_begin_marker_id IS NULL AND scope_begin_marker_digest IS NULL AND begun_at IS NULL AND evidence_digest IS NULL AND error_code IS NULL AND external_resource_id IS NULL AND external_resource_digest IS NULL AND superseded_by_intent_id IS NULL AND outcome_recorded_at IS NULL)
    OR (state='leased' AND lease_owner IS NOT NULL AND lease_expires_at IS NOT NULL AND lease_fence IS NOT NULL AND lease_fence_digest IS NOT NULL AND installation_begin_marker_id IS NULL AND installation_begin_marker_digest IS NULL AND scope_begin_marker_id IS NULL AND scope_begin_marker_digest IS NULL AND begun_at IS NULL AND evidence_digest IS NULL AND error_code IS NULL AND external_resource_id IS NULL AND external_resource_digest IS NULL AND superseded_by_intent_id IS NULL AND outcome_recorded_at IS NULL)
    ...
    OR (state='attention' AND begun_at IS NULL AND lease_owner IS NULL AND lease_expires_at IS NULL AND lease_fence IS NULL AND lease_fence_digest IS NULL AND installation_begin_marker_id IS NULL AND installation_begin_marker_digest IS NULL AND scope_begin_marker_id IS NULL AND scope_begin_marker_digest IS NULL AND external_resource_id IS NULL AND external_resource_digest IS NULL AND superseded_by_intent_id IS NULL AND evidence_digest IS NOT NULL AND error_code='delivery_deadline_exceeded' AND outcome_recorded_at IS NOT NULL)
    OR (state='superseded' AND lease_owner IS NULL AND lease_expires_at IS NULL AND lease_fence IS NULL AND lease_fence_digest IS NULL AND begun_at IS NULL AND installation_begin_marker_id IS NULL AND installation_begin_marker_digest IS NULL AND scope_begin_marker_id IS NULL AND scope_begin_marker_digest IS NULL AND external_resource_id IS NULL AND external_resource_digest IS NULL AND evidence_digest IS NOT NULL AND error_code='delivery_superseded' AND outcome_recorded_at IS NOT NULL AND superseded_by_intent_id IS NOT NULL))
);
