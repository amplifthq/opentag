...
import { TELEGRAM_DOCTOR_TITLE, TELEGRAM_STATUS_TITLE } from "./render.js";
import { type TelegramChannelBinding, normalizeTelegramMessage, stripTelegramInvocation } from "./normalize.js";
...
const HELP_TEXT = [
  "📚 OpenTag commands",
  "",
  "🔗 /bind <owner>/<repo> connects this Telegram chat to a Project Target.",
  "🔓 /unbind confirm disconnects this chat from the current Project Target.",
  "📌 /status shows the active run, queue, and next safe action.",
  "🩺 /doctor shows a redacted readiness summary.",
  "⏹️ /stop [run_id] requests cancellation; OpenTag never treats stop as successful completion.",
  "",
...
const UNBOUND_HINT =
  "🔗 This Telegram chat is not bound yet. Send /bind <owner>/<repo> before starting runs.";
const BIND_USAGE =
...
const BINDING_AUTH_DENIED_TEXT =
  "⚠️ Only an authorized Telegram binding manager can change this chat's Project Target. Ask an admin to run the command or update local OpenTag channel bindings.";
const STOP_UNAVAILABLE_TEXT = [
  "⏹️ Run cancellation from this Telegram ingress is not configured.",
  "OpenTag will not treat a stop request as successful completion. Use opentag status --run <run_id> for audit detail, or opentag service stop if you need to stop the local background service."
].join("\n");
...
function formatStopResultText(result: TelegramStopRunResult): string {
  ...
  if (result.outcome === "already_terminal") {
    return ["✅ Already finished", `🆔 Run: ${result.runId}`, "OpenTag will not change its final result."].join("\n");
  }
  return result.runId
    ? ["⚠️ Run not found", `🆔 Run: ${result.runId}`, "It may already be finished or no longer cancelable."].join("\n")
    : "⚠️ No active run was found for this Telegram chat and Project Target.";
}
    ...
    if (bindRequest) {
      if (!input.bindChannel) {
        await input.reply?.({
          ...
          chatId,
          messageId: message.message_id,
          text: "⚠️ Telegram chat binding from source threads is not configured. Re-run opentag setup or update local OpenTag channel bindings.",
          ...(message.message_thread_id ? { messageThreadId: message.message_thread_id } : {})
        });
      ...
      await input.reply?.({
        ...
        chatId,
        messageId: message.message_id,
        text: ["🔗 Bound", "", `📌 Target: ${bindRequest.repoProvider}:${bindRequest.owner}/${bindRequest.repo}`, "Send a task to start a run."].join(
          "\n"
        ),
        ...(message.message_thread_id ? { messageThreadId: message.message_thread_id } : {})
      });
    ...
    if (unbindRequest) {
      if (!input.unbindChannel) {
        await input.reply?.({
          ...
          chatId,
          messageId: message.message_id,
          text: "⚠️ Telegram chat unbinding from source threads is not configured. Update local OpenTag channel bindings instead.",
          ...(message.message_thread_id ? { messageThreadId: message.message_thread_id } : {})
        });
      ...
      await input.reply?.({
        ...
        chatId,
        messageId: message.message_id,
        text: ["🔓 Unbound", "", `Disconnected from ${formatProjectTarget(existingBinding)}.`, "Send /bind <owner>/<repo> to connect a new target."].join(
          "\n"
        ),
        ...(message.message_thread_id ? { messageThreadId: message.message_thread_id } : {})
      });
