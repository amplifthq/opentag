...
export const OFFICIAL_SETUP_LINKS = {
  ...
  discordInteractionsDocs: "https://discord.com/developers/docs/interactions/overview",
  discordSlashCommandsDocs: "https://discord.com/developers/docs/interactions/application-commands"
} as const;
...
function setupNeeds(platform: PlatformId, language: CliLanguage): string[] {
  if (language === "zh-CN") {
    switch (platform) {
      ...
      case "discord":
        return [
          "Discord Application ID、Public Key（基本信息页）和 Bot Token（机器人页）",
          "要绑定的 Discord 频道 ID（开发者模式下右键频道复制）",
          "Interactions 端点挂在 dispatcher 上，需要一个公网 tunnel 转发到本机 3030",
          "保存 tunnel URL 到 Developer Portal 的 Interactions Endpoint URL",
          "需要注册 /opentag 斜杠命令（配置教程含注册命令）"
        ];
      case "telegram":
        return [];
    }
  ...
  switch (platform) {
    ...
    case "discord":
      return [
        "Discord Application ID, Public Key (General Information) and Bot Token (Bot page)",
        "Discord channel ID to bind (right-click the channel with Developer Mode on)",
        "Interactions mount on the dispatcher; a public tunnel to local port 3030 is required",
        "Paste the tunnel URL into the Developer Portal Interactions Endpoint URL",
        "The /opentag slash command must be registered (the setup guide includes the command)"
      ];
    case "telegram":
      return [];
  }
...
function officialSetupLinks(platform: PlatformId, language: CliLanguage): string[] {
  ...
  switch (platform) {
    ...
    case "discord":
      return [
        `Discord Developer Portal: ${OFFICIAL_SETUP_LINKS.discordDeveloperPortal}`,
        `Interactions docs: ${OFFICIAL_SETUP_LINKS.discordInteractionsDocs}`,
        `Slash command docs: ${OFFICIAL_SETUP_LINKS.discordSlashCommandsDocs}`
      ];
    case "telegram":
      return [];
  }
...
export function formatDiscordCredentialHelp(language: CliLanguage): string {
  if (language === "zh-CN") {
    return [
      "Discord 凭证在哪里找:",
      `- 开发者后台: ${OFFICIAL_SETUP_LINKS.discordDeveloperPortal}`,
      "- Application ID 与 Public Key: 应用的 General Information（基本信息）页",
      "- Bot Token: 应用的 Bot（机器人）页，点 Reset Token 后仅显示一次",
      "",
      "保存后还需要两步（配置教程含具体命令）:",
      "- 起公网 tunnel 指向本机 dispatcher（默认 3030），把 <tunnel>/discord/interactions 填进 Interactions Endpoint URL",
      "- 注册 /opentag 斜杠命令"
    ].join("\n");
  }

  return [
    "Where to find the Discord credentials:",
    `- Developer Portal: ${OFFICIAL_SETUP_LINKS.discordDeveloperPortal}`,
    "- Application ID and Public Key: the app's General Information page",
    "- Bot Token: the app's Bot page; Reset Token shows it once",
    "",
    "Two steps remain after setup (the setup guide includes the exact commands):",
    "- Run a public tunnel to the local dispatcher (default 3030) and paste <tunnel>/discord/interactions into the Interactions Endpoint URL",
    "- Register the /opentag slash command"
  ].join("\n");
}
