    ...
    platformLines.push(
      ...(input.language === "zh-CN"
        ? [
            ...
            hostedOAuth ? "Webhook signing secret: 已在 trusted relay 上配置 OAuth App secret" : "Webhook signing secret: OpenTag 会自动生成",
            hostedOAuth ? "Webhook events: Comment 和 Agent Session events" : "Webhook events: Comment events"
          ]
        : [
            input.linear.teamKey ? `Linear team key: ${input.linear.teamKey}` : undefined,
            input.linear.teamId ? `Linear team id: ${input.linear.teamId}` : undefined,
            ...
            `Linear webhook URL: ${linearPublicWebhookUrlPlaceholder(webhookPath)}`,
            hostedOAuth ? "Webhook signing secret: configured on the trusted relay for the OAuth App" : "Webhook signing secret: generated by OpenTag",
            hostedOAuth ? "Webhook events: Comment and Agent Session events" : "Webhook events: Comment events"
...
export function formatSetupComplete(config: OpenTagCliConfig, configPath: string): string {
  ...
  const linearEventLine = linearHostedOAuth
    ? language === "zh-CN"
  ...
  if (language === "zh-CN") {
    return [
      ...
      linear && !linearHostedOAuth ? `Signing secret: ${linear.webhookSecret}` : undefined,
