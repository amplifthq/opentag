...
// Parse a run of one or more leading user mentions (e.g. a teammate mention
// placed before the bot mention) along with the whitespace separating them.
function leadingSlackMentions(text: string): { end: number; userIds: string[] } | null {
  const userIds: string[] = [];
  let cursor = 0;
  while (text.startsWith("<@", cursor)) {
    const close = text.indexOf(">", cursor + 2);
    if (close < 0) break;
    const body = text.slice(cursor + 2, close);
    if (!body) break;
    userIds.push(body.split("|", 1)[0] ?? "");
    cursor = close + 1;
    while (cursor < text.length && text[cursor]!.trim() === "") cursor += 1;
  }
  return userIds.length > 0 ? { end: cursor, userIds } : null;
}

export function stripSlackAppMention(text: string, botUserId?: string): string | null {
  const run = leadingSlackMentions(text);
  if (!run) return null;
  ...
  if (botUserId) {
    ...
    const expectedUserId = botUserId.toLowerCase();
    const botIsMentioned = run.userIds.some((id) => id.toLowerCase() === expectedUserId);
    if (!botIsMentioned) return null;
  }

  const stripped = text.slice(run.end).trim();
