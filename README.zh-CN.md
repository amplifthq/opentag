...
| 协议运行时 | 当前可用 | 工作线程、上下文包、审计轨迹、运行准入（run admission）、安静回调和指标 |
| Telegram 和 Lark | 实验性适配器 | 归一化器、入口 app 和回调辅助函数都已具备；把它们当作适配器扩展面，而不是 v0 的主黄金路径 |
| 托管多租户控制平面 | 未来加固 | 调度器目前刻意保持轻量；面向生产的多租户托管还需要更多运维加固 |

对于构建受治理工作流的团队，更深入的协议词汇都在 [Agent Work Protocol](docs/agent-work-protocol.md) 里：能力契约、策略解析、建议变更、审批、应用计划（apply plan）和血缘（lineage）。
