...
具体 setup 仍然会把 Slack、GitHub、GitLab、Linear、Lark / 飞书、Telegram 或 Discord 连接到本地 coding agent。但 OpenTag 不只是 connector：它保持 source-thread-native、local-first、executor-neutral，让工作留在已有上下文里，同时让 agent 看到了什么、被允许做什么、产出了什么、回调到了哪里都可以复盘。
...
3. 运行 opentag setup，并帮我选择 Slack、GitHub、GitLab、Linear、Lark / 飞书、Telegram 或 Discord、coding agent，以及本地项目。
...
```mermaid
flowchart LR
    A["Slack、GitHub、GitLab、Linear、Lark / 飞书、Telegram 或 Discord"] --> B["OpenTag listener"]
    B --> C["本地 dispatcher"]
    C --> D["本地 runner"]
