...
具体 setup 仍然会把 Slack、GitHub、GitLab、Lark / 飞书、Telegram、Discord 或 Microsoft Teams 连接到本地 coding agent。但 OpenTag 不只是 connector：它保持 source-thread-native、local-first、executor-neutral，让工作留在已有上下文里，同时让 agent 看到了什么、被允许做什么、产出了什么、回调到了哪里都可以复盘。
...
3. 运行 opentag setup，并帮我选择 Slack、GitHub、GitLab、Lark / 飞书、Telegram、Discord 或 Microsoft Teams、coding agent，以及本地项目。
...
| Microsoft Teams | 使用 Azure Bot、Teams 租户和公网 HTTPS tunnel | [Microsoft Teams 配置](docs/platforms/teams.zh-CN.md) |
...
```mermaid
flowchart LR
    A["Slack、GitHub、GitLab、Lark / 飞书、Telegram、Discord 或 Microsoft Teams"] --> B["OpenTag listener"]
    B --> C["本地 dispatcher"]
    C --> D["本地 runner"]
