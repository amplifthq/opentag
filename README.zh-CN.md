...
- 模型建议的下一步 → 源线程回复 `apply 1` → 审批决策 → 应用计划 → GitHub PR、适配器写操作或 child run fallback
...
- **把模型建议变成安全动作** —— 最终回调可以渲染标签、review 请求、后续运行，或带标题、分支、变更文件、风险和执行上下文的 `create_pull_request` 动作；用户在同一个源线程里批准、应用、拒绝或继续。
...
| 领域 | 状态 | 说明 |
| --- | --- | --- |
| GitHub | 当前可用 | Issue 评论、PR review 评论、回调、从已准备 run branch 经过线程审批创建 pull request，以及配置后的旧式即时 PR 创建 |
